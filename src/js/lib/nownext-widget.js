var NowNextLater = function () {
  ...
  this.scheduleEndpointURL = '//users-atlas.metabroadcast.com/3.0/schedule.json?';
...
NowNextLater.prototype.loadFullscreen = function () {
  ...
  $widgetPanels.each(function (index) {
    var template = Handlebars.compile($('#fullscreen-template').html());
    $(this).append(template(dataForFullscreen[0][index]));
  });
  ...
  if (page < dataForFullscreen.length) {
    if (index < items.length) {
      ...
      nowNextLater.loadPanelTimeout = setTimeout(function () {
        ...
        var template = Handlebars.compile($('#fullscreen-template').html());
        $(items[index]).append(template(dataForFullscreen[page][index]));
